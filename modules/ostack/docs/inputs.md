...
| <a name="input_globalinfra_repo_name"></a> [globalinfra_repo_name](#input_globalinfra_repo_name) | Name of the global infra repo so that oStack can apply its settings to it (eg. branch protection, team access)<br>It must be created already on the default VCS provider.<br>Set to `null` if you don't want oStack to manage this repo at all. | `string` | `"global-infra"` | no |
